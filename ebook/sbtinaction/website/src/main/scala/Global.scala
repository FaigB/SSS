...
import com.preownedkittens.database._
...
object Application extends Controller {
  ...
  def showSelectedKittens(id1: String, id2: String, id3: String) = {
    import com.preownedkittens.Logic._
    val buyerPreferences = com.preownedkittens.BuyerPreferences(Set(id1, id2, id3))

    val kittensWithLikelihood = Kitten.all().map{ k =>
      (k, matchLikelihood(com.preownedkittens.Kitten(k.id, KittenAttribute.allForKitten(k).map("" + _.attributeId).toSet), buyerPreferences))
...
object Global extends App with GlobalSettings {
  new play.core.server.NettyServer(new StaticApplication(new File(".")), 9001)
